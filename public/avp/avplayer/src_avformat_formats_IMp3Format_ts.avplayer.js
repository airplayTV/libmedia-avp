"use strict";
(self["webpackChunkAVPlayer"] = self["webpackChunkAVPlayer"] || []).push([["src_avformat_formats_IMp3Format_ts"],{
...
/* harmony import */ var avutil_codecs_mp3__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! avutil/codecs/mp3 */ "./src/avutil/codecs/mp3.ts");
...
/* harmony import */ var common_util_is__WEBPACK_IMPORTED_MODULE_20__ = __webpack_require__(/*! common/util/is */ "./src/common/util/is.ts");
const cheap__fileName__0 = "src\\avformat\\formats\\IMp3Format.ts";
...
class IMp3Format extends _IFormat__WEBPACK_IMPORTED_MODULE_7__["default"] {
    ...
    async parseID3(formatContext, stream) {
        ...
        if (hasID3) {
            ...
            await _mp3_id3v2__WEBPACK_IMPORTED_MODULE_17__.parse(formatContext.ioReader, len, this.context.id3v2, stream.metadata);
            if (common_util_is__WEBPACK_IMPORTED_MODULE_20__.bigint(stream.metadata['com.apple.streaming.transportStreamTimestamp'])) {
                const mp3Context = stream.privData;
                mp3Context.nextDTS = (0,avutil_util_rational__WEBPACK_IMPORTED_MODULE_13__.avRescaleQ)(stream.metadata['com.apple.streaming.transportStreamTimestamp'], {
                    num: 1,
                    den: 90000
                }, stream.timeBase);
            }
    ...
    async readHeader(formatContext) {
        ...
        if (this.context.firstFramePos !== formatContext.ioReader.getPos()) {
            common_util_logger__WEBPACK_IMPORTED_MODULE_6__.warn(`skipping ${formatContext.ioReader.getPos() - this.context.firstFramePos} bytes of junk at ${this.context.firstFramePos}`, cheap__fileName__0, 189);
            this.context.firstFramePos = formatContext.ioReader.getPos();
        }
        ...
        stream.codecpar.profile = avutil_codecs_mp3__WEBPACK_IMPORTED_MODULE_12__.getProfileByLayer(mp3Context.frameHeader.layer);
        stream.codecpar.frameSize = avutil_codecs_mp3__WEBPACK_IMPORTED_MODULE_12__.getFrameSizeByVersionLayer(mp3Context.frameHeader.version, mp3Context.frameHeader.layer);
        stream.codecpar.sampleRate = avutil_codecs_mp3__WEBPACK_IMPORTED_MODULE_12__.getSampleRateByVersionIndex(mp3Context.frameHeader.version, mp3Context.frameHeader.samplingFrequency);
        stream.timeBase.num = 1;
        stream.timeBase.den = stream.codecpar.sampleRate;
        const channels = mp3Context.frameHeader.mode === 3 ? 1 : 2;
        stream.codecpar.chLayout.nbChannels = channels;
        const bitrate = BigInt(Math.floor(avutil_codecs_mp3__WEBPACK_IMPORTED_MODULE_12__.getBitRateByVersionLayerIndex(mp3Context.frameHeader.version, mp3Context.frameHeader.layer, mp3Context.frameHeader.bitrateIndex)));
        ...
        if (tag === 'Xing' || tag === 'Info') {
            ...
            if (fSize && this.context.fileSize) {
                ...
                if (fSize > this.context.fileSize && delta > min >> BigInt(4)) {
                    ...
                    common_util_logger__WEBPACK_IMPORTED_MODULE_6__.warn('invalid concatenated file detected - using bitrate for duration', cheap__fileName__0, 242);
                }
                else if (delta > min >> BigInt(4)) {
                    common_util_logger__WEBPACK_IMPORTED_MODULE_6__.warn('filesize and duration do not match (growing file?)', cheap__fileName__0, 245);
                }
            }
        ...
        if (this.context.firstFramePos !== formatContext.ioReader.getPos()) {
            common_util_logger__WEBPACK_IMPORTED_MODULE_6__.warn(`skipping ${formatContext.ioReader.getPos() - this.context.firstFramePos} bytes of junk at ${this.context.firstFramePos}`, cheap__fileName__0, 305);
            this.context.firstFramePos = formatContext.ioReader.getPos();
        }
    ...
    async readAVPacket(formatContext, avpacket) {
        ...
        try {
            ...
            const frameLength = _mp3_frameHeader__WEBPACK_IMPORTED_MODULE_18__.getFrameLength(mp3Context.frameHeader, stream.codecpar.sampleRate);
            const frameSize = avutil_codecs_mp3__WEBPACK_IMPORTED_MODULE_12__.getFrameSizeByVersionLayer(mp3Context.frameHeader.version, mp3Context.frameHeader.layer);
            ...
            cheap_ctypeEnumWrite__WEBPACK_IMPORTED_MODULE_3__.CTypeEnumWrite[17](avpacket + 48, BigInt(Math.floor(frameSize)));
            ...
            mp3Context.nextDTS += BigInt(Math.floor(frameSize));
            ...
            if (formatContext.ioReader.error !== -1048576 /* IOError.END */
                ...
                common_util_logger__WEBPACK_IMPORTED_MODULE_6__.error(error.message, cheap__fileName__0, 360);
            }
    ...
    async seek(formatContext, stream, timestamp, flags) {
        ...
        if (flags & 16 /* AVSeekFlags.TIMESTAMP */ && (formatContext.ioReader.flags & 2 /* IOFlags.SLICE */)) {
            const seekTime = (0,avutil_util_rational__WEBPACK_IMPORTED_MODULE_13__.avRescaleQ)(timestamp, stream.timeBase, avutil_constant__WEBPACK_IMPORTED_MODULE_10__.AV_MILLI_TIME_BASE_Q);
        ...
        if (flags & 2 /* AVSeekFlags.BYTE */) {
            ...
            if (stream.sampleIndexes.length) {
                ...
                if (index > 0 && (0,avutil_util_rational__WEBPACK_IMPORTED_MODULE_13__.avRescaleQ)(timestamp - stream.sampleIndexes[index - 1].pts, stream.timeBase, avutil_constant__WEBPACK_IMPORTED_MODULE_10__.AV_MILLI_TIME_BASE_Q) < BigInt(10000)) {
                    common_util_logger__WEBPACK_IMPORTED_MODULE_6__.debug(`seek in sampleIndexes, found index: ${index}, pts: ${stream.sampleIndexes[index - 1].pts}, pos: ${stream.sampleIndexes[index - 1].pos}`, cheap__fileName__0, 475);
            ...
            if (this.context.isVBR) {
                if (mp3Context.tocIndexes.length) {
                    ...
                    if (sample) {
                        common_util_logger__WEBPACK_IMPORTED_MODULE_6__.debug(`seek in xing toc indexes, pts: ${sample.dts}, pos: ${sample.pos}`, cheap__fileName__0, 491);
                        await formatContext.ioReader.seek(sample.pos);
                        mp3Context.nextDTS = sample.dts;
                    }
                    else {
                        common_util_logger__WEBPACK_IMPORTED_MODULE_6__.debug('not found any keyframe index, try to seek in bytes', cheap__fileName__0, 496);
                        const frameLength = _mp3_frameHeader__WEBPACK_IMPORTED_MODULE_18__.getFrameLength(mp3Context.frameHeader, stream.codecpar.sampleRate);
                    ...
                    common_util_logger__WEBPACK_IMPORTED_MODULE_6__.debug('not found any keyframe index, try to seek in bytes', cheap__fileName__0, 505);
                    const frameLength = _mp3_frameHeader__WEBPACK_IMPORTED_MODULE_18__.getFrameLength(mp3Context.frameHeader, stream.codecpar.sampleRate);
                    const frame = timestamp / BigInt(stream.codecpar.frameSize >> 0);
                    const pos = frame * BigInt(Math.floor(frameLength)) + this.context.firstFramePos;
                    mp3Context.nextDTS = frame * BigInt(stream.codecpar.frameSize >> 0);
...
/* harmony export */ });
/* unused harmony export XING_SIZE */
/*
 * libmedia mp3 utils
 *
 * 版权所有 (C) 2024 赵高兴
...
//# sourceMappingURL=src_avformat_formats_IMp3Format_ts.avplayer.js.map