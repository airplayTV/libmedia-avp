...
type PointerType<T, Type = void> = T extends pointer<infer U> ? PointerType<U, U> : (Type extends void ? any : (Type extends BuiltinType ? `${Type['zzztype__']}*` : (Type extends string ? `${Type}*` : Type)));
...
declare type BuiltinType = i32 | i64 | f32 | f64 | uint8 | uint16 | uint32 | uint64 | int8 | int16 | int32 | int64 | float | float64 | double | char | bool | atomic_char | atomic_uint8 | atomic_uint16 | atomic_uint32 | atomic_int8 | atomic_int16 | atomic_int32 | atomic_uint64 | atomic_int64 | atomic_bool | size;
declare type AtomicType2Type<T> = T extends atomic_char ? char : T extends atomic_int16 ? int16 : T extends atomic_int32 ? int32 : T extends atomic_int8 ? int8 : T extends atomic_uint8 ? uint8 : T extends atomic_uint16 ? uint16 : T extends atomic_uint32 ? uint32 : T extends atomic_uint64 ? uint64 : T extends atomic_int64 ? int64 : T extends atomic_bool ? bool : never;
 ...
 * 位域
 */
declare type bit<T extends BuiltinType, U extends number> = T & {
    __bitfiled__?: true;
};
/**
 * 内联结构体
